...
namespace Unity.VRTemplate
{
    ...
    public class VideoTimeScrubControlRawImage : MonoBehaviour
    {
        [SerializeField]
        ...
        public String m_IconPlay;

        [SerializeField]
        [Tooltip("Pause icon sprite")]
        public String m_IconPause;

        [SerializeField]
        [Tooltip("Play or pause button image.")]
        public TextMeshProUGUI m_ButtonPlayOrPauseIcon;
        ...
        {   
            ...
            if (!m_VideoPlayer.playOnAwake) // if not playOnAwake
            {
                //m_VideoPlayer.playOnAwake = true; // Set play on awake for next enable.
                m_VideoPlayer.Play(); // Play video to load first frame.
                m_VideoPlayer.Stop(); // Stop Video Again
                VideoStop(); // Stop the video to set correct state and pause frame.
                m_ButtonPlayOrPause.SetActive(true); // Activate Play Pause button to indicate that video is paused
        ...
        {
            m_ButtonPlayOrPauseIcon.text = isPlaying ? "\uF04C" : "\uF04B"; // Pause : Play
            m_ButtonPlayOrPause.SetActive(!isPlaying);
        }

        public string GetCallerName()
        {
            var stackTrace = new StackTrace();
            var frame = stackTrace.GetFrame(2); // 0 = this method, 1 = FunctionB, 2 = FunctionA
